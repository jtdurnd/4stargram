...
export async function showPost(target_Id, sw) {
    ...
    if (sw === 1) {
        ...
        result = await client.db(dbname).collection("posts").find(query).project(projection).toArray();
    } else if (sw === 0) {

        //나랑 follow 관계인 사람들을 배열로 받아서 query.
        let first_query = { following_userID: `${target_Id}` };
        let first_projection = { follower_userID: 1 };
        let followerList = await client.db(dbname).collection("follower").find(first_query).project(first_projection).toArray();

        // followerList 는 현재 객체배열 --> 즉, index와 _id값을 모두 가지고 있는 배열이므로, 
        // 여기서 follower_userID값만 추출해서 다시 배열을 만들어 넣어준다. 
        let follower_userIDs = followerList.map(follower => follower.follower_userID);
        ...
        result = await client.db(dbname).collection("posts").find(follower_query).project(projection).toArray();
